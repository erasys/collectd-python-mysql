...
# Added support for Multi-Source replication by Jan Kunzmann <jan.kunzmann@erasys.de>
...
def get_metrics(conf):
	...
	for key in mysql_status:
		...
		dispatch_value(instance, 'status', key, mysql_status[key], ds_type)

	mysql_variables = fetch_mysql_variables(conn)
	for key in mysql_variables:
		dispatch_value(instance, 'variables', key, mysql_variables[key], 'gauge')

	mysql_master_status = fetch_mysql_master_stats(conn)
	for key in mysql_master_status:
		dispatch_value(instance, 'master', key, mysql_master_status[key], 'gauge')

	mysql_states = fetch_mysql_process_states(conn)
	for key in mysql_states:
		dispatch_value(instance, 'state', key, mysql_states[key], 'gauge')
	...
	for connection in slave_status:
		for key in slave_status[connection]:
			...
			dispatch_value(instance, prefix, key, slave_status[connection][key], 'gauge')

	response_times = fetch_mysql_response_times(conn)
	for key in response_times:
		dispatch_value(instance, 'response_time_total', str(key), response_times[key]['total'], 'counter')
		dispatch_value(instance, 'response_time_count', str(key), response_times[key]['count'], 'counter')

	innodb_status = fetch_innodb_stats(conn)
	for key in MYSQL_INNODB_STATUS_VARS:
		if key not in innodb_status: continue
		dispatch_value(instance, 'innodb', key, innodb_status[key], MYSQL_INNODB_STATUS_VARS[key])
